...
int sim900a_connect( void) 
{
	...
	while(sim900a_send_cmd("AT+CGREG?","+CGREG: 0,1",500))//检测是否应答AT指令 
	{
	...
	while(sim900a_send_cmd("AT+CGATT?","+CGATT: 1",500))//检测是否应答AT指令 
	{
  ...
  //AT+CSTT
	...
	while(sim900a_send_cmd(temp_buf,"OK",500))//检测是否应答AT指令 
	{
	...
	while(sim900a_send_cmd("AT+CIPSHUT","SHUT OK",500))//检测是否应答AT指令 
	{
