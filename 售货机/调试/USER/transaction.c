...
unsigned char Commodity_ReserveCheck(unsigned char id)
{
    ...
    if(wait_time != 0) //收到响应	,拆包数据
    {
        if((USART1_RX_STA & 0x8000) != 0)
        {
            ...
            if(TcpServer_ReadDataBuf1[0] == id)
            {
                Device_Transaction_Info.Transaction_Num = TcpServer_ReadDataBuf1[1];
                Device_Transaction_Info.Transaction_Price = TcpServer_ReadDataBuf1[2];
...
unsigned char CheckPay_Status(unsigned char id)
{
    ...
    if(wait_time != 0) //收到响应	,拆包数据
    {
        if((USART1_RX_STA & 0x8000) != 0)
        {
            ...
            if(TcpServer_ReadDataBuf1[0] == id)
            {
                Device_Transaction_Status.Transaction_Status = TcpServer_ReadDataBuf1[1];
                Device_Transaction_Status.Transaction_BuyRequest = TcpServer_ReadDataBuf1[2];
                Device_Transaction_Status.Transaction_PayStatus = TcpServer_ReadDataBuf1[3];
...
unsigned char PushBuy_Request(unsigned char id)
{
    ...
    mb_check.Reg_Num = 1;
    mb_check.Data_Buf[0] = 0x00;
    mb_check.Data_Buf[1] = 0x01;
...
{
    //根据键值确定商品编码
    Device_Transaction_Info.Transaction_ID = Key2CDID(Input_Key);
    //查询商品库存
