...
//发送缓存
volatile unsigned int TcpServer_WriteDataBuf[SEND_DATA_LEN];
//接收缓存
volatile unsigned int TcpServer_ReadDataBuf[RECV_DATA_LEN];
...
int modbus_send_cmd( volatile mb_info_struct *info_struct)
{
		...
		modbus_cmd_ReadKeepReg)||(info_struct->Func_Code==modbus_cmd_WriteMultiReg))
	{
		...
		{
			USART_SendData( USART2,mb_send_buffer[i]);
...
int recv_modbus_response( unsigned char *data)
{
	...
	//
	memset((unsigned int*)TcpServer_ReadDataBuf,0,sizeof(TcpServer_ReadDataBuf));
	//
	data_index=7;
	//判断功能码是否有效
		...
		switch( data[data_index])
		{
			...
			//计算寄存器数量
			  reg_num=(*(data+8))>>1;
				for( i=1;i<=reg_num;i++)
				{							
					TcpServer_ReadDataBuf[i-1]=*(data+9+(i-1)*2);//高位数据写入
					TcpServer_ReadDataBuf[i-1]<<=8;//高位左移8位
					TcpServer_ReadDataBuf[i-1]|=*(data+10+(i-1)*2);//低位数据写入
				}
				break;
