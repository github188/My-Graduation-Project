...
void iCAN_wrMsg(iCAN_msg *msg)
{
  ...
  {
  CAN1->sTxMailBox[0].TIR &= TMIDxR_TXRQ;
	...
	CAN1->sTxMailBox[0].TIR |=  TMIDxR_TXRQ;//CAN_TI1R_TXRQ;
	}
}
/**
...
uint8_t iCAN_Transmit(CAN_TypeDef* CANx, iCAN_msg* msg)
{
  uint8_t transmit_mailbox = 0;
	vu32 tId;
  /* Check the parameters */
  ...
  if (transmit_mailbox != CAN_TxStatus_NoMailBox)
  {
	...
	tId=tId | (msg->SourceID<<3); 
	tId=tId | 4;//CAN_ID_EXT; 
 	CANx->sTxMailBox[transmit_mailbox].TIR=tId;
	CANx->sTxMailBox[transmit_mailbox].TDLR = (((unsigned int)msg->data[3] << 24) | 
	                         ((unsigned int)msg->data[2] << 16) |
	                         ((unsigned int)msg->data[1] <<  8) | 
	                         ((unsigned int)msg->data[0])        );
	CANx->sTxMailBox[transmit_mailbox].TDHR = (((unsigned int)msg->data[7] << 24) | 
	                         ((unsigned int)msg->data[6] << 16) |
	                         ((unsigned int)msg->data[5] <<  8) |
	                         ((unsigned int)msg->data[4])        );
	                                              // Setup length
	msg->len &= (uint8_t)0x0000000F;
	CANx->sTxMailBox[transmit_mailbox].TDTR &= (uint32_t)0xFFFFFFF0;
	CANx->sTxMailBox[transmit_mailbox].TDTR |= msg->len;
	CANx->sTxMailBox[transmit_mailbox].TIR |=  TMIDxR_TXRQ;//CAN_TI1R_TXRQ;
	}
   return transmit_mailbox;
}
