...
extern void SendAllMd(void);
extern void SendRIO(void);
extern void WrWarnLed(unsigned char id,unsigned char wDat);
extern void ReIO(void);
...
int main(void)
{
  ...
  uint8_t tDataAndTimeArray[6];
	SystemInit();
  //NVIC_SetVectorTable(NVIC_VectTab_FLASH,0x08010000-NVIC_VectTab_FLASH);//矢量偏移
  ...
  while(1)
  {
		...
		if(flg1ms)
		{
			lngHz++;	Dis_Lcd(); KeyHandler();//ReNewDIO();
			flg1ms=0;	
		}
		if(0)//if(sys_adj50ms==1)
		{
		 if(frmNum==FRM_MAIN)
		 		Main_Con(dualChNum);
			sys_adj50ms=0;
		}
		if(0)//if(flgFvMbCrcOk==1)
		{
			flgFvMbCrcOk=0;
			chkFvMsg();
		}
		if(0)//if(flgFvMbTmrOut==1)
		{
			FvId=0;flgFvMbTmrOut=0;u6NewTxdFlg=0;
			SendFvCmd(FvId);
		}
    if(0)//if(flg1S==1)
    {
			...
			//SendAllMd();
			cntFvConn++;cntFmConn++;
			SendValv(cntValv);cntValv++;cntValv=cntValv&0x01;
			SendRIO();
			Dis_RtDat();RxdEvents();/**/TickEvents();//WrWarnLed(0,1);
			tDateAndTimeString = RTCGetDateAndTimeString(tDataAndTimeArray);
			Dis_Date();Dis_Time();
    }
		if(0)//if(NewPendBuf[25]==0xFA)//下发提单
		{
			Del_Pend(addChNum-1,DelPendBuf);	 
			AddNewPendLst(addChNum-1);	
		  u8PendNum[0]=dGetPendNum(0);u8PendNum[1]=dGetPendNum(1);//u8PendNum=GetPendNum();
		  if(sys_Conn==1)
				Dis_1_Pend(1);
		}
		if(0)//if((NewPendBuf[25]==0x5A)||(NewPendBuf[25]==0xAA))
		{
			tDat=Fnd_Pend(addChNum-1,NewPendAscBuf);
			if(tDat==0)
			{
				NewPendBuf[25]=0;
			}
		...
		if(0)//if(icBuf[9]==0xFA)//
		{	 	 Rfid();	 }
		if(0)//if(CardUsr[5]==0xFA)//检查用户卡
		{    ChkUsrCard();}
		
		if(0)//if(flg250ms)
		{
			flg250ms=0;
			if(((frmNum==FRM_MAIN)||(frmNum==FRM_MAN))&&(sys_STA==0))//	(sys_Conn==1)&&
			{
				if(cntRfid==0)
					cntRfid=1;
				if(++cntIC>=5)
				{
					...
					if(WarnOnOff[9]==0)//不指定鹤位
					{
						...
						SendCard(cntRfid);//提油密码
				}
			}
		if(0)//	if((frmNum==FRM_LOGIN)||(frmNum==FRM_MON))
			{
				if(++cntIC>=1)
				{
					cntIC=0;
