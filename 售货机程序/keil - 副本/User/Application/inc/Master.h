...
#include <stdint.h>
typedef struct  {
  unsigned char  fvConn;
  unsigned long  diagFV;           // 诊断字,长整数
  				float  fvSSL;            // 瞬时量
  				float  fvMD;             // 密度
  				float  fvWD;             // 温度
  				float  fvLJSm;           // 累积质量
					float  fvLJSv;					 // 累积体积
					float  fvLJSm_s;				 // 累积质量库存
					float  fvLJSv_s;				 // 累积体积库存
} FV_MSG;
typedef struct  {
  unsigned char  pTmr;
  unsigned char  pConn;
...
void CalWei(void);
float CalIEEE754(unsigned char dt1,unsigned char dt2,unsigned char dt3,unsigned char dt4,unsigned char tFlg);
